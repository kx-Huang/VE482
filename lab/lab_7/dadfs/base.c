...
/* A super block lock that must be used for any critical section operation on
 * the sb, such as: updating the free_blocks, inodes_count etc. */
...
static int dadfs_sb_get_objects_count(struct super_block *vsb, uint64_t *out) {
  struct dadfs_super_block *sb = DADFS_SB(vsb);
...
#endif
{
  ...
  if (pos) {
    /* FIXME: We use a hack of reading pos to figure if we have filled in all
     * data. We should probably fix this to work in a cursor based model and
     * use the tokens correctly to not fill too many data in each cursor based
     * call */
    return 0;
  }
  ...
  if (unlikely(!S_ISDIR(sfs_inode->mode))) {
    printk(KERN_ERR "inode [%llu][%lu] for fs object [%s] not a directory\n",
           sfs_inode->inode_no, inode->i_ino, filp->f_dentry->d_name.name);
    return -ENOTDIR;
  }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0)
    dir_emit(ctx, record->filename, DADFS_FILENAME_MAXLEN, record->inode_no,
             DT_UNKNOWN);
...
struct dadfs_inode *dadfs_get_inode(struct super_block *sb, uint64_t inode_no) {
...
int dadfs_inode_save(struct super_block *sb, struct dadfs_inode *sfs_inode) {
  ...
  inode_iterator =
      dadfs_inode_search(sb, (struct dadfs_inode *)bh->b_data, sfs_inode);
...
/* FIXME: The write support is rudimentary. I have not figured out a way to do
 * writes
 * from particular offsets (even though I have written some untested code for
 * this below) efficiently. */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0)
...
#endif
    brelse(bh);
    printk(KERN_ERR "Error copying file contents from the userspace buffer to "
                    "the kernel space\n");
    return -EFAULT;
  }
...
static int dadfs_create(struct inode *dir, struct dentry *dentry, umode_t mode,
                        bool excl);

static int dadfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode);
...
static int dadfs_create_fs_object(struct inode *dir, struct dentry *dentry,
  ...
  if (ret) {
    ...
    /* TODO: Remove the newly created inode from the disk and in-memory inode
     * store and also update the superblock, freemaps etc. to reflect the same.
...
int dadfs_fill_super(struct super_block *sb, void *data, int silent) {
  ...
  if (unlikely(sb_disk->magic != DADFS_MAGIC)) {
    printk(KERN_ERR "The filesystem that you try to mount is not of type "
                    "dadfs. Magicnumber mismatch.");
    goto release;
  }
  ...
  printk(KERN_INFO "dadfs filesystem of version [%llu] formatted with a block "
                   "size of [%llu] detected in the device.\n",
         sb_disk->version, sb_disk->block_size);

  /* A magic number that uniquely identifies our filesystem type */
  sb->s_magic = DADFS_MAGIC;

  /* For all practical purposes, we will be using this s_fs_info as the super
   * block */
  ...
  root_inode->i_private = dadfs_get_inode(sb, DADFS_ROOTDIR_INODE_NUMBER);
...
static struct dentry *dadfs_mount(struct file_system_type *fs_type, int flags,
...
static void dadfs_kill_superblock(struct super_block *sb) {
  printk(KERN_INFO "dadfs superblock is destroyed. Unmount succesful.\n");
...
static int dadfs_init(void) {
  ...
  sfs_inode_cachep =
      kmem_cache_create("sfs_inode_cache", sizeof(struct dadfs_inode), 0,
...
static void dadfs_exit(void) {
  ...
  if (likely(ret == 0))
    ...
    printk(KERN_ERR "Failed to unregister dadfs. Error:[%d]", ret);
}
